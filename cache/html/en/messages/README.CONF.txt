...
Whether to request a new copy of a page if the request from the browser has 'Pragma: no-cache' (default=yes).  This option should be set to 'no' if when browsing offline all pages are re-requested by a 'broken' browser.
ITEM cache-control-no-cache
[<URL-SPEC>] cache-control-no-cache = yes | no
Whether to request a new copy of a page if the request from the browser has 'Cache-Control: no-cache' or 'Cache-Control: max-age=0' (default=yes). This option can be set to 'no' for certain URL-specs to reduce the number of outgoing requests that are generated by a 'broken' browser. This option was added by Paul Rombouts.
...
ITEM max-size
max-size = (size)
The maximum size for the cache in MB after purging (default=-1).  A maximum cache size of -1 (or 0 for backwards compatability) means there is no limit to the size.  If this and the min-free options are both used the smaller cache size is chosen.  This option take into account the URLs that are never purged when measuring the cache size but will not purge them.
ITEM min-free
min-free = (size)
