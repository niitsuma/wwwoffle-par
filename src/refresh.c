 ...
 for(i=0;args[i];i++)
   {
    ...
    else if(!strcmp_litbeg(args[i],"iframes="))
       iframes=(args[i][strlitlen("iframes=")]=='Y');
 ...
 if(Url->args)
   {
    ...
    for(i=0;args[i];i++)
	 ...
	 recurse_limit=TrimArgs(URLDecodeFormArgs(args[i]+strlitlen("limit=")));
          ...
          recurse_iframes=(args[i][strlitlen("iframes=")]=='Y')*2;
       else if(!strcmp_litbeg(args[i],"scripts="))
...
void DefaultRecurseOptions(URL *Url,Header *head)
{
 ...
 if((value=GetHeader2Val(head,"Pragma","wwwoffle-iframes")))
    recurse_iframes=atoi(value);
       ...
       if(!IsLocalHost(iframeUrl) && IsProtocolHandled(iframeUrl))
         {
          ...
          if(recurse_depth>=0)
             if(!*recurse_limit || !strcmp_beg(iframeUrl->name,recurse_limit))
 ...
 else
   {
    ...
    if(new_outgoing==-1)
    ...
    else
      {
       ...
       if(recurse_force)
          AddToHeaderCombined(new_request_head,"Pragma","no-cache");

       sprintf(str,"wwwoffle-stylesheets=%d",recurse_stylesheets);
       AddToHeaderCombined(new_request_head,"Pragma",str);
                        ...
                        int stylesheets,int images,int frames,int iframes,int scripts,int objects)
{
 ...
 if(iframes)     len+=strlitlen(";iframes=Y");
...

 {
   ...
   if(iframes)     p=stpcpy(p,";iframes=Y");
